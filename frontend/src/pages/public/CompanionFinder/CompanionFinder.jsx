  ...
  return (
    <>
      ...
      >
        ...
        <div className="d-flex justify-content-center">
          <button
            ...
            data-bs-toggle={currentUser && currentUser.user_type === 'tourist' ? "modal" : undefined}
            data-bs-target={currentUser && currentUser.user_type === 'tourist' ? "#createPostModal" : undefined}
            onClick={() => {
              if (!currentUser) {
                if (onNavigate) {
                  onNavigate('auth');
              ...
              } else if (currentUser.user_type !== 'tourist') {
                alert("Only tourists can post travel plans.");
              }
            }}
          >
      ...
      <div className="container py-5">
                        ...
                        <div>
                          <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                            ...
                            <span className="badge bg-light text-dark border px-3 py-2 rounded-pill small">
                              Need: <strong className="text-emerald">{post.companions_needed - (post.accepted_count || 0)}</strong> more companions
                            </span>
                          </div>
                          ...
                          <div className="d-flex flex-wrap gap-2 mb-3">
                            ...
                            {post.trip_rating_count > 0 ? (
                              <span className="badge bg-warning bg-opacity-20 text-dark px-3 py-2 rounded-pill small">
                                <i className="bi bi-star-fill text-warning me-1"></i> Trip Rating: {post.trip_rating}/10 ({post.trip_rating_count})
                              </span>
                            ) : (
                              <span className="badge bg-secondary bg-opacity-10 text-secondary px-3 py-2 rounded-pill small">
                                <i className="bi bi-star me-1"></i> Unrated Trip
                              </span>
                            )}
                        ...
                        <div className="pt-3 border-top d-flex justify-content-end align-items-center mt-2">
                          {currentUser && currentUser.id == post.owner_id ? (
                            ...
                            <button
                              ...
                              data-bs-toggle={currentUser && currentUser.user_type === 'tourist' && (post.gender_preference === 'Any' || currentUser.gender === post.gender_preference) ? "modal" : undefined}
                              data-bs-target={currentUser && currentUser.user_type === 'tourist' && (post.gender_preference === 'Any' || currentUser.gender === post.gender_preference) ? "#requestJoinModal" : undefined}
                              onClick={() => {
                                if (!currentUser) {
                                  if (onNavigate) {
                                    onNavigate('auth');
                                  ...
                                  alert("Only tourists can request to join travel plans.");
                                } else if (post.gender_preference !== 'Any' && currentUser.gender !== post.gender_preference) {
                                  alert(`This trip is open to ${post.gender_preference} companions only. You cannot join.`);
                                } else {
                    ...
                    {myPosts.map((post) => (
                      <div className="col-md-6" key={post.id}>
                        <div className="card glass-card border-0 p-4">
                          ...
                          <p className="mb-1"><strong>Companions:</strong> {post.accepted_count || 0} / {post.companions_needed} accepted ({post.companions_needed - (post.accepted_count || 0)} more needed)</p>
                          <p className="mb-1"><strong>Budget:</strong> LKR {post.budget_range}</p>
                          <p className="mb-2" style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{post.description}</p>
                          {new Date(post.end_date) < new Date() && (
                            <div className="mt-3 border-top pt-2 d-flex justify-content-between align-items-center">
                              <span className="small text-muted"><i className="bi bi-calendar-check-fill text-success"></i> Trip Ended</span>
                              <button
                                className="btn btn-xs btn-outline-gradient rounded-pill px-3 py-1"
                                data-bs-toggle="modal"
                                data-bs-target="#rateCompanionsModal"
                        ...
                        <div className="card glass-card border-0 p-4">
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <div className="d-flex gap-2 align-items-center">
                              {request.trip_rating_count > 0 && (
                                <span className="badge bg-warning text-dark">
                                  ★ {request.trip_rating}/10
                                </span>
                              )}
                              <span className={`badge rounded-pill px-3 py-2 ${request.status === 'pending' ? 'bg-warning text-dark' : request.status === 'accepted' ? 'bg-success' : 'bg-danger'}`}>
                                {request.status}
                          ...
                          <p className="mb-2 text-truncate">{request.message || 'No message provided.'}</p>
                          {request.status === 'accepted' && new Date(request.end_date) < new Date() && (
                            <div className="mt-3 border-top pt-2 d-flex justify-content-between align-items-center">
                              <span className="small text-muted"><i className="bi bi-calendar-check-fill text-success"></i> Trip Ended</span>
                              <button
                                className="btn btn-xs btn-outline-gradient rounded-pill px-3 py-1"
                                data-bs-toggle="modal"
                                data-bs-target="#rateCompanionsModal"
                                onClick={() => handleOpenRateModal(request.post_id)}
                                style={{ fontSize: '12px' }}
                              >
                                <i className="bi bi-star-fill text-warning me-1"></i> Rate Companions
                              </button>
        ...
        <div className="modal fade" id="rateCompanionsModal" tabIndex="-1" aria-hidden="true">
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content rounded-4 border-0">
              <div className="modal-header border-0 pb-0">
                <h4 className="modal-title fw-bold text-gradient">Rate Travel Companions</h4>
                <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
